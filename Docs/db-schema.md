...
**Хотя бы одна роль у аккаунта есть всегда.** Пустой список — это не «клиент», а человек, которому ничего не доступно, включая собственные записи. Схема этого не проверяет (в SQL такое правило потребовало бы триггера на каждое удаление), это делает код: снятие последней роли отклоняется.
