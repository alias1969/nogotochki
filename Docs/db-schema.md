...
**Изменения в 2.1:** роли стали списком — таблица `user_roles` (миграция 007). У одного человека может быть несколько ролей одновременно, например мастер и администратор. Поле `users.role` удалено: два источника правды о правах — это та самая «ошибка в правах доступа» из паспорта. `sessions.role_at_login` стало `roles_at_login`. См. [раздел 4.21](#421-user_roles--роли-пользователя) и [раздел 7.20](#720-роли-списком-а-не-одним-полем).
**Изменения в 2.0:** истёкшие сессии удаляются, а не копятся вечно. Добавлен индекс `ix_sessions_expires` (миграция 006) — по нему работает уборщик, как `ix_holds_expires` у резервов. См. [раздел 7.13](#713-сессии-в-базе-а-не-самодостаточный-токен).
...
| 21 | `user_roles` | Роли пользователя списком: у одного человека их может быть несколько |
...
Единая таблица для всех ролей. Клиент, мастер и администратор входят по e-mail и паролю, а чем каждый из них вправе заниматься, сказано не здесь, а в [`user_roles`](#421-user_roles--роли-пользователя): ролей у одного аккаунта может быть несколько.
...
| `password_hash` | TEXT | нет | Хеш пароля вместе с солью и параметрами стойкости: `scrypt$n=16384,r=8,p=1$<соль>$<хеш>`. Соль своя у каждого пароля. `NULL` — клиент заведён администратором вручную и вход ещё не активирован |
...
| `roles_at_login` | TEXT | да | Роли на момент входа — список через запятую, по алфавиту: `admin,master`. Расхождение с текущими ролями закрывает сессию |
...
- **Роли зафиксированы на момент входа.** Если администратор поменял роли на экране A10, старая сессия не должна молча получить новые права: при расхождении `roles_at_login` с текущим списком из `user_roles` сессия закрывается и требуется повторный вход. Список хранится строкой — отсортированной и через запятую, — потому что он нужен целиком и только для сравнения с таким же списком; разбирать его на части никто не будет.
...
### 4.21. `user_roles` — роли пользователя

Роли человека — список, а не одно значение. У владелицы студии есть и кабинет мастера, и админ-панель: она и принимает клиентов, и ведёт прайс. С одним полем `role` пришлось бы выбирать, чем ей быть сегодня, или заводить второй аккаунт с другим e-mail — и тогда её записи и её выручка разъезжаются по двум учётным записям.

Права проверяются вопросом «есть ли роль в списке», а не «совпадает ли роль». Разница не косметическая: равенство `role === 'master'` закрывает кабинет мастера перед той самой владелицей, у которой роль администратора стоит «сильнее».

| Поле | Тип | Обяз. | Описание |
|---|---|---|---|
| `user_id` | INTEGER | PK, FK | → `users.id`, каскадное удаление вместе с аккаунтом |
| `role` | TEXT | PK | `user` / `master` / `admin` |
| `granted_at` | TEXT | да | Когда роль выдана, UTC |
| `granted_by` | INTEGER | нет | FK → `users.id` — кто выдал. `NULL` у ролей, заведённых миграцией и регистрацией |

Первичный ключ `(user_id, role)` — одна и та же роль не выдаётся дважды, и выдача роли, которая уже есть, ничего не ломает.

`granted_by` обнуляется, а не удаляет строку, если уволенного администратора убрали из базы: роль выдана по-настоящему, и терять её вместе с тем, кто её выдал, нельзя. Кто именно выдал — остаётся в `audit_log`.

**Хотя бы одна роль у аккаунта есть всегда.** Пустой список — это не «клиент», а человек, которому ничего не доступно, включая собственные записи. Схема этого не проверяет (в SQL такое правило потребовало бы триггера на каждое удаление), это делает код, и на двух рубежах: снятие последней роли отклоняется при правке, а аккаунт, у которого ролей всё же не осталось, не пускают ни вход, ни проверка сессии. Второй рубеж нужен потому, что снимок ролей в сессии такую дыру не ловит — пустая строка совпадает с пустой строкой.
...
| 34 | `ix_sessions_expires (expires_at)` | Уборка истёкших сессий при каждом входе | Уборщик перебирает все сессии студии. `ix_sessions_user` здесь не помогает: в нём `user_id` стоит первым, а уборка идёт по всем пользователям сразу |
| 35 | `ix_user_roles_role (role, user_id)` | A10 — список по роли; проверка «остался ли хоть один администратор» | Обратный вопрос «у кого есть эта роль» первичным ключом не обслуживается: в `PRIMARY KEY (user_id, role)` роль стоит второй. Проверка последнего администратора выполняется на каждой правке роли |
...
**Истёкшая сессия удаляется.** Доступа она не даёт и так — проверка берёт только строки с `expires_at > now`, — но строка хранит хеш токена и строку браузера, а таблица от входа к входу только растёт. Уборка идёт при открытии новой сессии, по индексу `ix_sessions_expires`: там, где сессии и так пишутся. Так же устроена уборка резервов (`sweepExpiredHolds`).

Отозванная сессия — выход, смена пароля, смена роли — удаляется тем же уборщиком, когда доходит до своего исходного срока. Отдельного правила для неё нет: `revoked_at` прекращает доступ сразу, а строка убирается вместе со всеми прочими мёртвыми. На `sessions` не ссылается ни один внешний ключ, поэтому удаление ничего за собой не тянет.
...
### 7.20. Роли списком, а не одним полем

**Альтернатива:** одно поле `users.role` и проверки вида `role === 'admin'`.

**Выбрано:** таблица `user_roles` и проверка «есть ли роль в списке».

Одно поле исходит из того, что человек в студии занимает ровно одну должность. В маленькой студии это неверно с первого дня: владелица и принимает клиентов, и ведёт прайс. Обходных путей два, и оба плохи. Второй аккаунт на другой e-mail разрывает историю: её записи в одном кабинете, её выручка — в другом, а клиент видит в списке мастеров человека, которого в отчётах нет. Ручное переключение роли туда-сюда превращает права в режим работы, который легко забыть переключить обратно.

Цена решения — дополнительный запрос при опознании сессии и то, что политики действий (кто может создать запись, кто отметить исход) больше не выбираются одним обращением к полю. Правило выбора пришлось сформулировать явно: **человек действует в самой сильной из своих ролей, которой это действие разрешено.** Администратор-мастер создаёт запись как администратор, потому что администратору это разрешено, а мастеру нет. Мастер-клиент записывается сам как клиент — мастеру запись создавать нельзя, и роль клиента оказывается единственной подходящей.

**У этого правила два уточнения, и оба появились не сразу.** Проверка безопасности показала, что «сильнейшая роль» сама по себе даёт дыру: доступ к записи складывается по всем ролям, а действие идёт от одной, и на пересечении мастер, записавшийся к коллеге, получал доступ к записи *как клиент*, а действовал над ней *как мастер*.

1. **Роль действия должна быть связана с объектом.** Мастер отмечает исход только по записям своего расписания — проверяется по карточке мастера (`masters.user_id`), а не по наличию роли. Иначе он дописывал бы данные в отчёт о загрузке и выручке другого мастера, и журнал показывал бы это как действие мастера, не уточняя, что мастер был не тот.
2. **Над записью, где человек сам клиент, действуют правила клиента.** Срок отмены и лимит переносов применяются к нему, кем бы он ни работал: студия теряет слот одинаково, кем бы ни был отменивший за час до визита. Это же правило применяет показ записи, поэтому кнопки на экране и ответ сервера не расходятся.

Поле `users.role` удалено, а не оставлено «для совместимости». Два источника правды о правах расходятся молча, и расхождение обнаруживается не на тесте, а в тот день, когда клиент увидел чужие записи. Это прямо названный в паспорте риск: права прописываются в API вручную, без готового механизма вроде RLS.

**Видимость записей от ролей не зависит напрямую — она складывается.** Список ролей не выбирает одну ветку проверки, а объединяет всё, что разрешает каждая роль: администратору видны все записи, мастеру — записи его расписания, клиенту — те, где он клиент. У человека с двумя ролями доступ равен объединению, а не сильнейшей из них: мастер, записавшийся к коллеге, обязан видеть и свой визит тоже.
...
|---|---|---|
| `users.password_hash` | Хеш пароля (scrypt) вместе с индивидуальной солью и параметрами стойкости | нет — хеш |
