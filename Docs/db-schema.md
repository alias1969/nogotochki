...
### 7.9. Уникальный индекс не ловит пересечения интервалов

**Ограничение, а не выбор, но о нём нужно знать.**
...
Поэтому проверка пересечений обязана быть в коде API внутри одной транзакции: `BEGIN IMMEDIATE`, проверка пересечений, вставка, `COMMIT`. Индекс остаётся последним рубежом на случай ошибки в коде, а не единственной защитой. Это прямо связано с риском из паспорта: «если проверка доступности не реализована на уровне API, возможна двойная запись».
