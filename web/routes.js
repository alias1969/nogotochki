...
export const SCREENS = {
  ...
  // Админ-панель. Вход в раздел — чистый адрес /admin, без имени файла:
  // serve.mjs (и в проде — раздатчик статики) отдаёт по нему файл
  // admin.html, так же, как по «/» отдаётся index.html. Остальные экраны
  // раздела — обычные файлы рядом, по тому же правилу, что и везде
  // в web/: «admin/appointments.html» не читается — файлы раздела ссылаются
  // друг на друга относительным именем, а не префиксом «admin/», и
  // подкаталог сломал бы это для страниц за его пределами (например,
  // login.js, который тоже ссылается на A2).
  A2: 'admin',                      // Админ — Сегодня
  A4: 'admin-appointments.html',    // Админ — Записи
  A5: 'admin-services.html',        // Админ — Услуги
  A6: 'admin-masters.html',         // Админ — Мастера
  A7: 'admin-schedules.html',       // Админ — Графики
  A8: 'admin-clients.html',         // Админ — Клиенты
  A10: 'admin-roles.html',          // Админ — Роли и доступы
  A11: 'admin-settings.html',       // Админ — Настройки
};

/**
 * Куда вести после входа с учётом роли.
 *
 * Роль проверяется вопросом «есть ли admin в списке ролей» — так же,
 * как на сервере (server/src/lib/roles.js: has() ищет вхождение,
 * а не сравнивает role === 'admin'). У владелицы студии кроме роли
 * администратора может быть и роль мастера или клиента одновременно,
 * и именно наличие admin в списке решает, куда её вести — в /admin,
 * а не в личный кабинет.
 */
export function afterAuthHref(roles) {
  return Array.isArray(roles) && roles.includes('admin') ? SCREENS.A2 : AFTER_AUTH;
}
